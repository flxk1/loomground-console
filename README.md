...
The **terminal face of [host](https://github.com/flxk1/host)** — a branded
first-run flow, a deterministic governance **chat**, an ASCII **dashboard**, and
optional bring-your-own-key LLM assist.

It is a **client, not an engine.** All governance logic lives in host; this
console drives host's surface over **MCP** — the same seam other clients use.
...
loomground-console workspaces      # the workspaces host knows
...
Point it at your host install directory; it defaults to `~/host`.

## How it works

- **Deterministic by default** — chat routes through host's governance engine
  (an audited intent router + dispatch). **No LLM** unless you `connect` your own
  provider (BYOK). When connected, the model only *phrases* host's result — it
...
- host's browser console is the other face.
